    ...
    public class BasicEventCommands : InteractionModuleBase<InteractionContext>
    {
        ...
        public async Task SetTriggerTime(IVoiceChannel voiceChannel, string tempChannelName = "Temp Voice")
        {
            ...
            if (tempChannel != null)
            {
                ...
                {
                    properties.Content = $"Cannot assign the Trigger Voice Channel to a Temporary Voice Channel.";
                });
                return;
            }
            ...
            await ModifyOriginalResponseAsync(properties =>
            {
                properties.Content = $"{voiceChannel.Mention} assigned as Trigger Channel.";
            });
        }

        [EnabledInDm(false)]
        [DefaultMemberPermissions(GuildPermission.Administrator)]
        [SlashCommand("remove", "Removes a given voice channel as a TriggerChannel", runMode: RunMode.Async)]
        public async Task RemoveTriggerChannel(IVoiceChannel voiceChannel)
        {
            await DeferAsync(true);

            var db = _Services.GetRequiredService<IDatabase>();
            var triggerChannel = await db.GetAsync<VoiceChannelData>($"TriggerChannel/{voiceChannel.Id}");
            if (triggerChannel == null)
            {
                await ModifyOriginalResponseAsync(properties =>
                {
                    properties.Content = $"{voiceChannel.Mention} is not a TriggerChannel.";
                });
                return;
            }

            await db.DeleteAsync($"TriggerChannel/{voiceChannel.Id}");

            List<VoiceChannelData> tempChannels = new();
            await db.GetAllAsync<VoiceChannelData>($"TempChannel", (path, data) =>
            {
                tempChannels.Add(data);
                return false;
            });

            foreach (VoiceChannelData data in tempChannels)
            {
                var channel = await Context.Guild.GetChannelAsync(data.ChannelId);
                if(channel != null)
                {
            ...
            await ModifyOriginalResponseAsync(properties =>
            {
                properties.Content = $"{voiceChannel.Mention} removed as TriggerChannel. All temp channels generated by this has been removed.";
            });
        }
